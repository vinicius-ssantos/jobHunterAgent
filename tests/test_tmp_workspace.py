...
class WorkspaceTmpDirTests(unittest.TestCase):
    ...
    def test_prepare_workspace_tmp_dir_prunes_only_older_same_prefix_directories(self) -> None:
        ...
        with patch("tests.tmp_workspace._PRUNE_GRACE_SECONDS", 0.0), patch(
            "tests.tmp_workspace._RECENT_DIRS_TO_KEEP",
            2,
        ):
    ...
    def test_prepare_workspace_tmp_dir_skips_recent_same_prefix_directories(self) -> None:
        root = Path.cwd() / ".tmp-tests"
        root.mkdir(parents=True, exist_ok=True)
        prefix = "tmp-workspace-test"
        recent = []
        for suffix in ("recent-a", "recent-b", "recent-c"):
            path = root / f"{prefix}-{suffix}"
            path.mkdir(parents=True, exist_ok=True)
            recent.append(path.name)

        prepare_workspace_tmp_dir(prefix)

        remaining_dirs = {path.name for path in root.iterdir() if path.is_dir() and path.name.startswith(prefix)}
        self.assertTrue(set(recent).issubset(remaining_dirs))
