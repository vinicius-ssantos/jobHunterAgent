...
class MatchingWorkerExecutionTests(IsolatedAsyncioTestCase):
    async def test_run_matching_worker_once_emits_scored_events_and_is_idempotent(self) -> None:
        ...
        try:
            ...
            self.assertEqual(
                ...
                {
                    "JobScoring:v1:run_id=77:external_key=k1",
                    "JobScoring:v1:run_id=77:external_key=k2",
                },
            )
    ...
    async def test_run_matching_worker_once_respects_legacy_state_keys(self) -> None:
        temp_dir = prepare_workspace_tmp_dir("matching-worker-legacy-state")
        input_path = temp_dir / "collected.ndjson"
        output_path = temp_dir / "scored.ndjson"
        state_path = temp_dir / "state.json"
        input_event = {
        ...
        state_path.write_text(
            json.dumps({"processed_event_ids": ["77:k1"]}, ensure_ascii=False),
            encoding="utf-8",
        )
        settings = type("Settings", (), {"minimum_relevance": 6})()
        try:
            rendered = await run_matching_worker_once(
                input_path=input_path,
                output_path=output_path,
                state_path=state_path,
                settings=settings,
            ...
            self.assertIn("emitidos=0", rendered)
            self.assertIn("duplicados_ignorados=1", rendered)
            state_payload = json.loads(state_path.read_text(encoding="utf-8"))
            self.assertIn("77:k1", state_payload["processed_event_ids"])
            self.assertIn(
                "JobScoring:v1:run_id=77:external_key=k1",
                state_payload["processed_event_ids"],
            )
        finally:
            for path in (input_path, output_path, state_path):
                if path.exists():
